    ...
    if (mode == -1) {
        ...
        if (status == 0) {
            if (cm.isQuestCompleted(100007)) {
                ...
                cm.sendNext("嗯...这封信是在#b汉斯#k那里带来的吗？......所以你来这里参加测试，打算进行第二次转职吗？好吧，我来给你解释一下测试过程。别担心，没那么复杂。");
            } else {
                cm.sendOk("一旦你准备好了，我可以送你进去。");
                cm.dispose();
            }
        } else if (status == 1) {
            cm.sendNextPrev("我会送你进入魔法师2转考验场，在那里你会看到平时看不到的怪物，它们看起来和平常一样，但是不能为你提供经验值，也不会掉落物品。");
        } else if (status == 2) {
