...
function start() {
    ...
    if (!(cm.getPlayer().getLevel() >= 70 && jobBase == jobStyle && cm.getJobId() % 10 == 0)){
        ...
        cm.sendNext("你是想探索扎昆副本吗？但你不是飞侠，我没办法对你进行判断。你去找你职业对应的长老吧。");
        cm.dispose();
        return;
    }
...
function action(mode, type, selection){
    ...
    if (actionx["Mental"]){
        ...
        if (status == 0)
            ...
            cm.sendNextPrev("这是考试的后半部分。这项测试将决定你是否足够聪明，能够迈出迈向成功的下一步。在奥西里亚的雪原上有一个被雪覆盖的黑暗区域，叫做圣地，连怪物都无法到达。在这个区域的中心有一块巨大的石头，叫做圣石。你需要提供一个特殊的项目作为祭品，然后圣石将测试你的智慧当场。");
        } else if (status == 2)
