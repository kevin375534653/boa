...
function start() {
    ...
    if (returnmap == -1)
        returnmap = 104000000;
    cm.sendNext("所以你想离开#b#m110000000##k?如果你愿意，我可以带你回#b#m"+returnmap+"##k.");
}
...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        cm.sendNext("如果你想回去的时候再联系我.");
        cm.dispose();
        return;
    } else if (mode == 1) {
        status++;
        if (status == 1)
            cm.sendYesNo("你确定要返回到#b#m"+returnmap+"##k?好吧，我们得快点。你想回#m"+returnmap+"#吗?")
        else {
