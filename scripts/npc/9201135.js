...
function start() {
    if (cm.getPlayer().getMap().getId() != 540000000) {
        text = "嘿，我是#p9201135#，是#r马来西亚#k导游，你想去哪里旅行？\n\n";
    } else {
        text = "嘿，我是#p9201135#，#r马来西亚#k导游。由于你没有在我们的合作伙伴#b冒险旅行社#k的特别旅行社中登记，这趟车要贵得多。那么，你现在想出行吗？\n\n";
        startedTravel = true;
    }
...
function action(mode, type, selection) {
    ...
    if (status == 1) {
        ...
        if (travelCost > 0) {
            cm.sendYesNo("你想去#b#m" + travelMap + "##k旅行吗? 需要花费#r" + cm.numberWithCommas(travelCost) + "金币#k。你想现在就走吗？");
        } else {
            cm.sendNext("在#r马来西亚#k玩的开心吗？期待着你的平安归来！ ");
        }
    } else if (status == 2) {
        if (cm.getMeso() < travelCost) {
            cm.sendNext("你好像没有足够的金币。");
        } else {
            if (travelCost > 0) {
