...
function start() {
    ...
    if (!(cm.getPlayer().getLevel() >= 70 && jobBase == jobStyle && cm.getJobId() % 10 == 0)){
        ...
        cm.sendNext("你是想探索扎昆副本吗？但你不是海盗，我没办法对你进行判断。你去找你职业对应的长老吧。");
        cm.dispose();
        return;
    }
        ...
        if (sel == 0){
            if (cm.getPlayer().getLevel() >= 70 && cm.getJobId() % 10 == 0){
                if (status == 0)
                    ...
                    cm.sendNext("很好。你将在海盗的两个重要方面受到考验：力量和智慧。我现在给你解释一下测试的物理部分。还记得诺特勒斯号的#b#p1090000##k吗？去见他，他会告诉你考试前半部分的细节。请完成任务，然后从#p1090000#把#b#t4031057##k带回来给我.");
                } else if (status == 2)
