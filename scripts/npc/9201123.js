        ...
        if (status == 0) {
            if (cm.getJob() == "BEGINNER") {
                if (cm.getLevel() >= 10 && cm.canGetFirstJob(jobType)) {
                    cm.sendYesNo("嘿#h#，我可以送你去#b#m" + map + "##k，如果你想成为一个#b" + job + "#k。你现在想去吗？");
                } else {
                    cm.sendOk("如果你想成为一个#b" + job + "#k，继续训练自己，直到你达到#b10级" + cm.getFirstJobStatRequirement(jobType) + "#k。");
                    cm.dispose();
                }
            } else {
                cm.sendOk("你现在强大多了。继续训练！");
                cm.dispose();
            }
        } else if (status == 1) {
            cm.warp(map, 0);
