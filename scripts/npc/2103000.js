    ...
    if (mode < 0) {
        ...
        if (status == 0 && mode == 1) {
            if (cm.isQuestStarted(3900) && cm.getQuestProgressInt(3900) != 5) {
                cm.sendOk("#b(我尝过绿洲的水。甘甜清爽的水帮我解了渴。)", 2);
                ...
                if (cm.canHold(2210005)) {
                    if (!cm.haveItem(2210005) && !isTigunMorphed(cm.getPlayer())) {
                        ...
                        cm.sendOk("绿洲的水一直都这么清澈。仔细看的话能清澈见底。#t2210005#", 2);
                    }
                } else {
                    cm.sendOk("你没有可用的背包空间。", 2);
                }
            } else if (cm.isQuestStarted(3934) || (cm.isQuestCompleted(3934) && !cm.isQuestCompleted(3935))) {
                if (cm.canHold(2210005)) {
                    if (!cm.haveItem(2210005) && !isTigunMorphed(cm.getPlayer())) {
                        cm.gainItem(2210005, 1);
                        cm.sendOk("你设法在河上找到了一个奇怪的瓶子。它看起来像是一个能变成警卫提干的样子的变身药水，使用它，你可以在进入到沙子图团。", 2);
                    }
                } else {
                    cm.sendOk("你的背包满了，清理背包后再来和我对话。", 2);
                }
            }
