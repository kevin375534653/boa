...
function start() {
    ...
    if (!(cm.getPlayer().getLevel() >= 70 && jobBase == jobStyle && cm.getJobId() % 10 == 0)){
        ...
        cm.sendNext("你是想探索扎昆副本吗？但你不是弓箭手，我没办法对你进行判断。你去找你职业对应的长老吧。");
        cm.dispose();
        return;
    }
	    ...
	    if (sel == 0){
	        if (cm.getPlayer().getLevel() >= 70 && cm.getJobId() % 10 == 0) {
	            if (status == 0)
	                ...
	                cm.sendNext("很好。你将受到两个重要方面的考验：力量和智慧。我现在给你解释一下测试的物理部分。你还记得射手村的#b#p1012100##k吗?去见他，他会告诉你考试前半部分的细节。请完成任务，然后从#p1012100#把#b#t4031057##k带回来给我.");
	            } else if (status == 2)
