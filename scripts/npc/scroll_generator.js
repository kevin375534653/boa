...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if(status == 0) {
            cm.sendNext("这是 MapleTV Scroll Generator 广播。 放置您在冒险过程中获得的补给品或金币来兑换奖品！ 您可以放置#任意数量的补给品#k，但请注意，将#r不同的补给品#k与其中任何一个的#rbigger镜头一起放置#k将提高获得奖励的可能性！");
        } else if(status == 1) {
            var sendStr;

            //print("Book: " + sgBookBuckets + " Item: " + sgItemBuckets);
            
            if(sgItemBuckets > 0.0) sendStr = "对于您当前放置的物品，您有 #r" + sgBuckets + "#k 个存储桶 (#r" + (sgItemBuckets < 1.0 ? sgItemBuckets.toFixed(2) : Math.floor(sgItemBuckets)) + "#k 个供应存储桶) 领取奖品。 放置用品：";
            else sendStr = "您还没有放置任何补给品。 放置用品：";
            ...
            cm.sendSimple(sendStr + "\r\n\r\n" + listStr + "#r#L" + (sgItems.length + 2) + "#领取奖品！#l#k\r\n");
        } else if(status == 2) {
            if(selection == (sgItems.length + 2)) {
                if(sgItemBuckets < 1.0) {
                    cm.sendPrev("您设置的补给不足。 插入至少一桶#b补给品#k 即可领取奖品。");
                } else {
                ...
                if(curItemQty > 0) {
                    cm.sendGetText("您想提供多少个“+tickSel+”？ (#r" + curItemQty + "#k 可用)#k");
                } else {
                    cm.sendPrev("您已获得 #rnone#k " + tickSel + " 来提供滚动生成。 单击“#r返回#k”返回主界面。");
                }
            }
        } else if(status == 3) {
            var text = cm.getText();
            ...
            try {
                ...
                if(placedQty > curItemQty) {
                    cm.sendPrev("无法插入给定量的 #r" + (curItemSel < sgItems.length ? "#t" + sgItems[curItemSel] + "#" : "金币") + "#k (#r" + curItemQty + "#k 可用的）。 单击“#r返回#k”返回主界面。");
                } else {
                    if(curItemSel < sgItems.length) sgApplyItem(curItemSel, placedQty);
                    else sgApplyMeso(placedQty);

                    cm.sendPrev("操作成功。 单击“#r返回#k”返回主界面。");
                }
            } catch(err) {
                cm.sendPrev("您必须输入要插入的正数耗材。 单击“#r返回#k”返回主界面。");
            }
    ...
    if (cm.getPlayer().getInventory(Packages.client.inventory.MapleInventoryType.USE).getNumFreeSlot() >= 1) {
        ...
        if (itemid != -1) {
            if (performExchange(itemid, 1)) {
                cm.sendNext("交易已接受！ 您已收到 #r#t" + itemid + "##k.");
            } else {
                cm.sendOk("哦，好像有些物品丢失了...请在尝试交换之前仔细检查库存中提供的物品。");
            }
        } else {
            cm.sendOk("抱歉给您带来不便，但目前商店中似乎没有卷轴...请稍后再试。");
        }
    } else {
        cm.sendOk("在尝试卷轴之前，请先留意您的消耗栏中是否有可用的栏位。");
    }
}
