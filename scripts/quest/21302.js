...
function end(mode, type, selection) {
    ...
    if (status == 0) {
        qm.sendNext("啊！这、这是……你终于想起制作#t4032312#的方法了？啊啊……这就是为什么你再笨再健忘，我也依然对你不离不弃……哎呀，现在不是说这些的时候！快把宝石给我！"); //Giant Polearm
    } else if (status == 1) {
        qm.sendNextPrev("好了，#t4032312#的力量应该可以恢复了，你的能力也需要再唤醒一些。现在你的等级升高了不少，可以被唤醒的能力也更多了！");
    } else if (status == 2) {
        if (!qm.isQuestCompleted(21302)) {
            if (!qm.canHold(1142131)) {
                qm.sendOk("嗯，你的#b装备栏#k已满。我需要您至少腾出1个空位才能完成此任务。");
                qm.dispose();
                return;
            }
        ...
        qm.sendNext("赶紧恢复以前的能力吧，带上我一起去冒险……");
    } else if (status == 3) {
        qm.dispose();
    }
}