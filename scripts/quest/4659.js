...
function end(mode, type, selection) {
    ...
    if (status == 0) {
        if (qm.getMeso() < 10000) {
            qm.sendOk("嘿！ 我需要 #b10,000 金币#k 来让你的宠物进化！");
            qm.dispose();
            return;
        }
        qm.sendNext("找到进化材料真是太棒了。 我现在给你一个机器人。");
    } else if (status == 1) {
        if (qm.isQuestCompleted(4659)) {
            qm.dropMessage(1, "这是怎么到这里的？");
            qm.dispose();
            ...
            if (closeness < 1642) {
                qm.sendOk("看起来你的宠物还没有长大到可以进化的程度。 多训练一点，直到它达到#b15级#k。");
                qm.dispose();
                return;
            }
