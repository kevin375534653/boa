...
function start(mode, type, selection) {
    ...
    if (mode != 1) {
        if (type == 15 && mode == 0) {
            qm.sendNext("哦，不需要拒绝我的提议。它只是一个药水。好吧，如果你改变主意了就告诉我。");
            qm.dispose();
            return;
        }
        //status -= 2;
    ...
    if (status == 0) {
        ...
        qm.sendAcceptDecline("但是可能是因为你被黑魔法师的诅咒困在冰里几百年时间，所以体力全都消耗完了。#b我给你一个体力恢复药水，请快点喝下去吧#k。");//nexon probably forgot to remove the '.' before '#k', lol
    } else if (status == 4) {
