...
function start(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
            qm.sendNext("你回来了，英雄。在#m200000000#的事情办得怎么样？确实是和黑色之翼有关吧？为什么表情这么凝重？说来听听。");
        } else if (status == 1) {
	qm.sendNextPrev("#b（讲述有关#m200000000#的封印石的事情。）#k", 2);
        } else if(status ==  2) {
            qm.sendNext("嗯。#m200000000#也有封印石啊……这倒是很重要的一个情报。虽说被抢走了很可惜……啊，我倒不是在怪你。没想到黑色之翼这次是有备而来啊。");
        } else if (status == 3) {
	qm.sendNextPrev("......", 2);
        } else if(status ==  4) {
            qm.sendNext("振作精神！对，就是这样。#p1201000#这次又解读出来了#b新技能#k。你去#b#m140000000#见见#p1201000#吧#k，正好连#m200000000#的消息也一起带过去。");
        } else if(status ==  5) {
