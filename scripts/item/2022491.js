...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
            if(im.getBossLogAlawys("新手礼包1") < 1 && im.getLevel()>= 1){//一级礼包
                im.setBossLog("新手礼包1");
                im.gainItem(2000000,100);//50血瓶
                im.gainItem(2000003,100);//50蓝瓶
                im.gainItem(2030000,10);//回城卷轴
                im.getPlayer().dropMessage(5,"一级礼包领取完毕");

            } else if(im.getBossLogAlawys("新手礼包10") < 1 && im.getLevel()>= 10){//10级礼包
                im.setBossLog("新手礼包10");

                im.gainItem(2000000,888);//50血瓶
                im.gainItem(2000003,888);//50蓝瓶
                im.gainItem(2030005,20);//回城卷轴
                im.gainItem(2030004,20);//回城卷轴
                im.gainItem(2030003,20);//回城卷轴
                im.gainItem(2030002,20);//回城卷轴
                im.gainItem(2030019,20);//回城卷轴

                im.getPlayer().dropMessage(5,"十级礼包领取完毕");
            } else if(im.getBossLogAlawys("新手礼包30") < 1 && im.getLevel()>= 10){//30级礼包
                im.setBossLog("新手礼包30");

                im.gainItem(2000016,999);//50血瓶
                im.gainItem(2000018,999);//50蓝瓶
                im.gainItem(2030005,20);//回城卷轴
                im.gainItem(2030004,20);//回城卷轴
                im.gainItem(2030003,20);//回城卷轴
                im.gainItem(2030002,20);//回城卷轴
                im.gainItem(2030019,20);//回城卷轴
                im.getPlayer().getCashShop().gainCash(1,8888)//给抵用券

                /**给枫叶武器*/

                im.getPlayer().dropMessage(5,"三十级礼包领取完毕");

                /**扣除箱子*/
                im.gainItem(2022491,-1);
            } else {
                im.getPlayer().dropMessage(5,"打开条件不足");
            }
