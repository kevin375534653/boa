...
function afterSetup(eim) {
    eim.dropMessage(5, "守护雕像的第一次攻击将在15秒后开始，请做好准备。");
...
function playerDead(eim, player) {
    ...
    if (count == 10) {
        eim.dropMessage(5, "[远征队]远征队员已经倒下了，品克缤是不可战胜的，挑战结束！");
        end(eim);
    } else if (count == 4) {
...
function spawnJrBoss(eim, mobid, gotKilled) {
    ...
    map.spawnMonsterOnGroundBelow(spawnid, 5, -46);
}
    ...
    if (isPinkBean(mob)) {
        ...
        if (noJrBossesLeft(mob.getMap())) {
            ...
            if (stage == 5) {
                ...
                eim.dropMessage(6, "随着女神雕像的倒下，品克缤失去了保护。真正的战斗现在开始！");
                eim.schedule("spawnPKB", 15 * 1000);
            } else {
                stage++;
                eim.setIntProperty("stage", stage);
                eim.dropMessage(5, "守护雕像的下一次攻击将在15秒后开始，请做好准备。");
                //eim.schedule("killStoneMonster", 5 * 1000); //可以让石像消失
                eim.schedule("startWave", 15 * 1000);
            }
        }
    }
...
function spawnPKB(eim) {
    var map = eim.getMapInstance(270050100);
    map.spawnMonsterOnGroundBelow(8820001, 5, -46);
}

function killStoneMonster(eim) {
    var mapObj = eim.getMapInstance(270050100);
    var stage = eim.getIntProperty("stage");
    if (stage == 2) {
        mapObj.killMonster(8820022);
        mapObj.killMonster(8820023);
        mapObj.killMonster(8820024);
        mapObj.killMonster(8820021);
    } else if (stage == 3) {
        mapObj.killMonster(8820019);
        mapObj.killMonster(8820024);
        mapObj.killMonster(8820025);
        mapObj.killMonster(8820026);
    } else if (stage == 4) {
        mapObj.killMonster(8820024);
        mapObj.killMonster(8820025);
        mapObj.killMonster(8820026);
        mapObj.killMonster(8820027);
    } else if (stage == 5) {
        mapObj.killMonster(8820023);
        mapObj.killMonster(8820024);
        mapObj.killMonster(8820025);
...
function startWave(eim) {
    ...
    for (var i = 1; i <= stage; i++) {
        spawnJrBoss(eim, (8820019 + (i % 5)), false);
    }
}
