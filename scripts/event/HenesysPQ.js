...
function bunnyDefeated(eim) {
    eim.dropMessage(5, "保护月妙失败！将移动到流放地。");
    end(eim);
}
...
function friendlyItemDrop(eim, mob) {
    if (mob.getId() == 9300061) {
        ...
        mob.getMap().broadcastMessage(PacketCreator.serverNotice(6, "月妙只做了第" + cakes + "个年糕。"));
    }
}
...
function friendlyDamaged(eim, mob) {
    if (mob.getId() == 9300061) {
        ...
        if (bunnyDamage > 5) {
            const PacketCreator = Java.type('tools.PacketCreator');
            broadcastMessage(PacketCreator.serverNotice(6, "月妙受了伤...请保护他避免受到攻击，让他能够专心制作年糕！"));
