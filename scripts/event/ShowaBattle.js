...
function playerEntry(eim, player) {
    eim.dropMessage(5, "[远征队] " + player.getName() + " 进入了地图。");
...
function changedMap(eim, player, mapid) {
    if (mapid < minMapId || mapid > maxMapId) {
        if (eim.isExpeditionTeamLackingNow(true, minPlayers, player)) {
            ...
            eim.dropMessage(5, "[远征队] 队长已经退出或者人数不足。");
            end(eim);
        } else {
            eim.dropMessage(5, "[远征队] " + player.getName() + " 已经离开了。");
            eim.unregisterPlayer(player);
        }
    }
}
...
function clearPQ(eim) {
    ...
    if (eim.getIntProperty("playerDied") == 0) {
        ...
        eim.dropMessage(5, "铃木：BOSS战败，没有人员伤亡，干得好！ 我们在里面发现了一台可疑的机器，我们正在把它搬出去。");
    }
}
