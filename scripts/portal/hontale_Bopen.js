...
function enter(pi) {
    ...
    if (pi.getPlayer().getMapId() == 240050101) {
        ...
        if (avail == null) {
            ...
            pi.getPlayer().dropMessage(6, "黑暗龙王封印了这扇门。");
            return false;
        } else {
            pi.playPortalSound();
        ...
        if (avail == null) {
            // do nothing; send message to player
            pi.getPlayer().dropMessage(6, "黑暗龙王封印了传送门。");
            return false;
        } else {
            pi.playPortalSound();
            pi.getPlayer().changeMap(target, targetPortal);
        ...
        if (avail == null) {
            if (pi.haveItem(4001092) && pi.isEventLeader()) {
                eim.showClearEffect();
                pi.getPlayer().dropMessage(6, "队长使用钥匙打破了封印。。。");
                pi.playPortalSound();
                pi.getPlayer().changeMap(target, targetPortal);
                eim.setIntProperty("5stageclear", 1);
                return true;
            } else {
                pi.getPlayer().dropMessage(6, "黑暗龙王封印了传送门。需要#b红色钥匙#k才能解除封印。");
                return false;
            }
        } else {
            pi.playPortalSound();
