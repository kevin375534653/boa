...
public class NPCScriptManager extends AbstractScriptManager {
    ...
    /**打开物品脚本*/
    public boolean itemStart(Client c,int itemId){
        return start(c, 9010000,-1,String.valueOf(itemId),c.getPlayer(),true, "im");
    }

    private boolean start(Client c, int npc, int oid, String fileName, Character chr, boolean itemScript, String engineName) {
        try {
            if(c.getPlayer().isGM()){
                c.getPlayer().dropMessage(5,"打开npc:"+npc);
            }
            ...
            log.error("Error starting NPC script: {}", npc, e);
            if(c.getPlayer().isGM()){
                c.getPlayer().dropMessage(5,"Error starting NPC script: {"+npc+":"+e+"}");
            }
