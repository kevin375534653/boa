...
public class NPCConversationManager extends AbstractPlayerInteraction {
    ...
    public void sendStyle(String text, int[] styles) {
        ...
        if (styles.length > 0) {
//            int[] c = {};
//            if(styles[0] > 40000 && styles[0]< 50000){
//                c = IntStream.concat(Arrays.stream(new int[]{30000}), Arrays.stream(styles)).toArray();
//            } else if(styles[0] >= 50000){
//                c = IntStream.concat(Arrays.stream(new int[]{20000}), Arrays.stream(styles)).toArray();
//            } else {
//                c = styles;
//            }
...
//        sendNext("您已获得 #b#t" + item.getId() + "##k.");
    ...
    public boolean sendCPQMapLists() {
        ...
        for (int i = 0; i < 6; i++) {
            if (fieldTaken(i)) {
                if (fieldLobbied(i)) {
                    msg += "#b#L" + i + "#怪物嘉年华场地 " + (i + 1) + " (Level: "  // "Carnival field" GMS-like improvement thanks to Jayd (jaydenseah)
                ...
                if (i >= 0 && i <= 3) {
                    msg += "#b#L" + i + "#怪物嘉年华场地 " + (i + 1) + " (2x2) #l\r\n";
                } else {
                    msg += "#b#L" + i + "#怪物嘉年华场地 " + (i + 1) + " (3x3) #l\r\n";
                }
            }
        }
    ...
    /**获取永久boss记录*/
    public int getBossLogAlawys(String boss) {
        Connection con = null;
        ...
        try {
            ...
            ps = con.prepareStatement("select count(*) as count from bosslog where characterid = ? and bossid = ?");
