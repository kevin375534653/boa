...
public class AbstractPlayerInteraction {
    ...
    public Item gainItem(int id, short quantity, boolean randomStats, boolean showMessage, long expires, Pet from) {
        ...
        if (quantity >= 0) {
            ...
            if (!InventoryManipulator.checkSpace(c, id, quantity, "")) {
                c.getPlayer().dropMessage(1, "道具栏已满．\r\n请整理" + ItemConstants.getInventoryType(id).name() + "窗口．");
                return null;
            }
    ...
    public int createExpedition(ExpeditionType type, boolean silent, int minPlayers, int maxPlayers) {
        ...
        if (!ExpeditionBossLog.attemptBoss(player.getId(), channel, exped, false) && !YamlConfig.config.server.USE_ENABLE_NO_NUM_EXPEDITIONS) {    // thanks Conrad for noticing missing expeditions entry limit
            return 1;
        }
