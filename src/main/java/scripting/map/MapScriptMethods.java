...
public class MapScriptMethods extends AbstractPlayerInteraction {
    ...
    public void explorerQuest(short questid, String questName) {
        ...
        if (status.equals(infoex)) {
            ...
            smp.append("获取称号。<").append(questName).append(">").append(rewardstring);
            getPlayer().sendPacket(PacketCreator.getShowQuestCompletion(quest.getId()));
        } else {
            getPlayer().sendPacket(PacketCreator.earnTitleMessage("探险了 "+ status + "/" + infoex + " 个地区"));
            etm.append("正在挑战称号 - ").append(questName).append("");
            smp.append("正在挑战称号 - ").append(questName).append("。").append(status).append("/").append(infoex).append("完成");
        }
    ...
    public void touchTheSky() { //29004
        ...
        getPlayer().sendPacket(PacketCreator.earnTitleMessage(status + "/5 地点完成"));
        getPlayer().sendPacket(PacketCreator.earnTitleMessage("正在挑战称号 - 站在巅峰的人"));
        if (Integer.toString(qs.getMedalProgress()).equals(qs.getInfoEx(0))) {
            showInfoText("The One Who's Touched the Sky" + rewardstring);
            getPlayer().sendPacket(PacketCreator.getShowQuestCompletion(quest.getId()));
        } else {
            showInfoText("正在挑战称号 - 站在巅峰的人" + status + "/5 完成");
        }
    }
}
