...
//        p.writeByte(YamlConfig.config.server.ENABLE_PIN && !c.canBypassPin() ? 0 : 1); // 0 = Pin-System Enabled, 1 = Disabled
        /**单独账号控制pin*/
        p.writeByte(c.getVerify()>1 && !c.canBypassPin() ? 0 : 1); // 0 = Pin-System Enabled, 1 = Disabled
//        p.writeByte(YamlConfig.config.server.ENABLE_PIC && !c.canBypassPic() ? (c.getPic() == null || c.getPic().equals("") ? 0 : 1) : 2); // 0 = Register PIC, 1 = Ask for PIC, 2 = Disabled
        /**单独账号控制pic*/
        p.writeByte(c.getVerify()>0 && !c.canBypassPic() ? (c.getPic() == null || c.getPic().equals("") ? 0 : 1) : 2); // 0 = Register PIC, 1 = Ask for PIC, 2 = Disabled
...
//        p.writeByte(YamlConfig.config.server.ENABLE_PIC && !c.canBypassPic() ? (c.getPic() == null || c.getPic().equals("") ? 0 : 1) : 2);
        p.writeByte(c.getVerify()>0 && !c.canBypassPic() ? (c.getPic() == null || c.getPic().equals("") ? 0 : 1) : 2);
    ...
    public static Packet gachaponMessage(Item item, String town, Character player) {
        ...
        p.writeByte(0x0B);
        p.writeString(player.getName() + " : "); //快乐百宝箱抽奖喇叭
        p.writeInt(0); //random?
        p.writeString(town);
    ...
    public static Packet requestBuddylistAdd(int chrIdFrom, int chrId, String nameFrom) {
        ...
        p.writeInt(chrIdFrom);
        p.writeFixedString(getRightPaddedStr("", '\0', 11));
        p.writeByte(0x09);
        p.writeByte(0xf0);
        p.writeByte(0x01);
        p.writeInt(0x0f);
    ...
    public static Packet triggerReactor(Reactor reactor, int stance) {
        if(reactor.getId() == 2618005 || reactor.getId() == 2618006){
            System.out.println("断点");
        }
//        if(reactor.getId() == 2618006){
//            stance = 2;
//        }
        ...
        p.writeByte(stance);
        p.writeShort(0);
        p.writeByte(5); // 帧延迟，设置为5，因为似乎没有固定的公式
        return p;
    }
    ...
    public static Packet sendDuey(int operation, List<DueyPackage> packages) {
        ...
        if (operation == 8) {
            ...
            for (DueyPackage dp : packages) {
                ...
                for (int i = dp.getSender().getBytes(CharsetConstants.CHARSET).length; i < 13; i++) {
                    p.writeByte(0);
                }
