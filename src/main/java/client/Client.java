...
public class Client extends ChannelInboundHandlerAdapter {
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        ...
        if (YamlConfig.config.server.USE_DEBUG_SHOW_RCVD_PACKET && !LoggingUtil.isIgnoredRecvPacket(opcode)) {
            log.debug("收到封包 {}", Integer.toHexString(opcode));
        }
        ...
        if (handler != null && handler.validateState(this)) {
            try {
                ...
                final String chrInfo = player != null ? player.getName() + " 在地图 " + player.getMapId() : "?";
                log.warn("错误封包处理 {}. 角色 {}, 账号 {}. 封包: {}", handler.getClass().getSimpleName(),
...
//        }
    ...
    /**获取账号验证状态
     * 密码状态验证 0不开启二级密码 1开启pic 2开启pin
     * */
    public int getVerify(){

        try (Connection con = DatabaseConnection.getConnection();
             PreparedStatement ps = con.prepareStatement("SELECT `verify` FROM accounts WHERE id = ?")) {
...
//        }
    ...
    public void changeChannel(int channel) {
        ...
        if (!player.isAlive() || FieldLimit.CANNOTMIGRATE.check(player.getMap().getFieldLimit())) {
            ...
            sendPacket(PacketCreator.serverNotice(1, "在这里做不了。"));
            sendPacket(PacketCreator.enableActions());
            return;
        }
