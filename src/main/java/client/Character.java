...
import cache.Cache;
import cache.MasterCache;
import client.active_query.ActiveQuery;
...
public class Character extends AbstractCharacterObject {
    ...
    /**记录活跃*/
    private Map<String, Integer> activeQueryMap = new ConcurrentHashMap<>();

    private String activeQueryDate = "";
    ...
    public static boolean canCreateChar(String name) {
        ...
        return getIdByName(name) < 0 && Pattern.compile("[A-Za-z0-9_\\-\\u4e00-\\u9fa5\\-\\u2e80-\\u9fff\\ue810-\\ue814]{2,12}").matcher(name).matches(); //中文支持
    }
    ...
    public void changeSkillLevelCreate(Skill skill, byte newLevel, int newMasterlevel, long expiration) {
        if (newLevel > -1) {
            skills.put(skill, new SkillEntry(newLevel, newMasterlevel, expiration));
            if (!GameConstants.isHiddenSkills(skill.getId())) {
//                sendPacket(PacketCreator.updateSkill(skill.getId(), newLevel, newMasterlevel, expiration));
...
//            sendPacket(PacketCreator.updateSkill(skill.getId(), newLevel, newMasterlevel, -1)); //Shouldn't use expiration anymore :)
                    ...
                    if ((MapId.isSelfLootableOnly(mapId))) {//happyville trees and guild PQ
                        if (!mapitem.isPlayerDrop() || mapitem.getDropper().getObjectId() == client.getPlayer().getObjectId()) {
                            if (mapitem.getMeso() > 0) {
                                ...
                                int nxGain = mapitem.getItemId() == ItemId.NX_CARD_100 ? 100 : 5000;
                                this.getCashShop().gainCash(1, nxGain);

                                if (YamlConfig.config.server.USE_ANNOUNCE_NX_COUPON_LOOT) {
                                    //showHint("You have earned #e#b" + nxGain + " NX#k#n. (" + this.getCashShop().getCash(1) + " NX)", 300);
                                    sendPacket(PacketCreator.getShowItemGain(mapitem.getItemId(), (short) 1));
                                }
    ...
    public void gainExp(int gain, int party, boolean show, boolean inChat, boolean white) {
        /**卡等级*/
        if(getLevel()>= Cache.openLevel){
            dropMessage("你已经到达卡级等级，不能再获取经验了");
            return;
        }
    ...
    public int getMaxClassLevel() {
        return (int) (isCygnus() ? YamlConfig.config.server.CYGNUS_MAXLEVEL : YamlConfig.config.server.MAXLEVEL);
    }
    ...
    public boolean hasMerchant() {
        return client.getWorldServer().hasHiredMerchant(this.id);
//        return hasMerchant;
    }
    ...
    public void showBuybackInfo() {
        String s = "#e复活状态 #n\r\n\r\n当前购买复活费用 : #b" + getBuybackFee() + " " + (YamlConfig.config.server.USE_BUYBACK_WITH_MESOS ? "金币" : "点券") + "#k\r\n\r\n";
        ...
        if (!isAlive()) {
            ...
            if (timeRemaining < 1) {
                ...
                s += "可复活倒计时 : #e#b" + getTimeRemaining(MINUTES.toMillis(YamlConfig.config.server.BUYBACK_RETURN_MINUTES) - timeLapsed) + "#k#n";
            }
            s += "\r\n";
        }

        if (timeNow < getNextBuybackTime() && avail) {
            s += "可以在 #r" + getTimeRemaining(getNextBuybackTime() - timeNow) + "后复活#k";
            s += "\r\n";
        } else {
            s += "复活 #b可用#k";
        }
    ...
    private void levelUpMessages() {
        ...
        if (level == 5) {
            ...
            */
            ...
            this.dropMessage(6, "[祝贺]" + getMedalText() + getName() + "终于达到了200级. 大家一起祝贺下吧。");
        }
    }
    ...
    public synchronized void resetStats() {
        ...
        try {
            ...
            if (tap >= 0) {
                ...
                log.warn("角色 {} 试图在没有足够AP可用的情况下重置其统计数据");
            }
        } finally {
            statWlock.unlock();
            effLock.unlock();
    ...
    public final boolean insertNewChar(CharacterFactoryRecipe recipe) {
        ...
        /**初始技能添加*/
//        recipe.addStartingSkillLevel(SkillFactory.getSkill(1004),1);//骑兽技能
...
//        this.changeSkillLevelCreate(SkillFactory.getSkill(1004), (byte) 1,1,-1);//骑宠技能

        /**初始物品添加*/
//        recipe.addStartingItem(1902107,1,InventoryType.EQUIP);//小b猪坐骑
        recipe.addStartingItem(2022491,1,InventoryType.USE);//新手礼包
        recipe.addStartingItem(5030000,1,InventoryType.CASH);//商店
...
//        this.getInventory(1).addItem(ii.getEquipById(1902107));//小b猪坐骑
//        this.getInventory(1).addItem(ii.getEquipById(1912107));//小b猪坐垫
    ...
    public synchronized void saveCharToDB(boolean notAutosave) {
        ...
        log.debug("正在尝试 {} 角色 {}", notAutosave ? "保存" : "自动保存", name);
        ...
        log.debug("成功 {} 角色 {}", notAutosave ? "保存" : "自动保存", name);
    }
    ...
    public void showUnderleveledInfo(Monster mob) {
        ...
        if (nextWarningTime < curTime) {
            ...
            showHint("你没有从击杀 #e#b" + mob.getName() + "#k#n (lv. #b" + mob.getLevel() + "#k) 中获得经验! 你要大致等级相同才可以.");
        }
    }
    ...
    public boolean runTirednessSchedule() {
        if (maplemount.getItemId() / 10000 == 193) {
            return true;
        }
        if (maplemount != null) {
        ...
        if (pendantOfSpirit == null) {
            pendantOfSpirit = TimerManager.getInstance().register(new Runnable() {
                ...
                public void run() {
                    if (pendantExp < 3) {
                        pendantExp++;
                        message("由于装备了精灵吊坠，打猎时额外获得" + pendantExp + "0%的经验值奖励。");
                    } else {
    ...
    public int getBossLogAcc(String boss) {
        ...
        try {
            ...
            ps = con.prepareStatement("select count(*) as count from bosslog_acc where accid = ? and bossid = ? and TO_DAYS(lastattempt) = TO_DAYS(NOW())");
            ps.setInt(1, client.getPlayer().getAccountID());
            ps.setString(2, boss);
            rs = ps.executeQuery();
            if (rs.next()) {
                ret_cout = rs.getInt("count");
            } else {
                ret_cout = -1;
            }
            rs.close();
    ...
    public int getBossLog(String boss) {
        ...
        try {
            con = DatabaseConnection.getConnection();
            ps = con.prepareStatement("select count(*) as count from bosslog where characterid = ? and bossid = ? and TO_DAYS(lastattempt) = TO_DAYS(NOW())");
    ...
    public void setBossLogAcc(String bossid) {
        ...
        try {
            con = DatabaseConnection.getConnection();
            ps = con.prepareStatement("insert into bosslog_acc (accid, bossid) values (?,?)");
            ps.setInt(1, client.getPlayer().getAccountID());
...
//             FileoutputUtil.outError("logs/数据库异常.txt", Ex);
    ...
    public List<List<Integer>> getFishingRewards(int type, int rate) { //获取钓鱼奖励池
        List<Integer> listS = null;
        List<List<Integer>> list = new ArrayList<List<Integer>>();
        try {
            Connection con1 = DatabaseConnection.getConnection();
            PreparedStatement ps;
            ps = con1.prepareStatement("select itemid,quantity from z_fishing where type=? and rate>=?");//钓鱼奖池添加到数据库。
            ps.setInt(1, type);
            ps.setInt(2, rate);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
    ...
    /**获取今日活跃*/
    public int getActiveAll(){
        int count = 0;
        try {

            Connection con = DatabaseConnection.getConnection();
            PreparedStatement ps;
            ps = con.prepareStatement("SELECT SUM(count) from ActiveQuery where accid = ? and today_=?");//获取活跃系统。
            ps.setInt(1,getAccountID());
            ps.setString(2,DateUtil.format(new Date(),"yyyy-MM-dd"));
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                count = rs.getInt(1);
            }
    ...
    /**根据Name获取活跃*/
    public int getActiveByName(String name){
        /**第二天重载活跃*/
        if(!activeQueryDate.equals(DateUtil.format(new Date(),"yyyy-MM-dd"))){
            loadActive();
        }

        int count = 0;
        if(activeQueryMap.containsKey(name)){
            count = activeQueryMap.get(name);
        }
//        try {
//
//            Connection con = DatabaseConnection.getConnection();
//            PreparedStatement ps;
//            ps = con.prepareStatement("SELECT SUM(count) from ActiveQuery where accid = ? and activeName = ?");//获取活跃系统。
//            ps.setInt(1,getAccountID());
//            ps.setString(2,name);
//            ResultSet rs = ps.executeQuery();
//            while (rs.next()) {
...
//
//        } catch (Exception Ex) {
//            log.error("错误 getActiveByName：", Ex);
//        }
        return count;
    ...
    /**根据type获取活跃*/
    public int getActiveByType(String type){
        int count = 0;
        try {

            Connection con = DatabaseConnection.getConnection();
            PreparedStatement ps;
            ps = con.prepareStatement("SELECT SUM(count) from ActiveQuery where accid = ? and activeType = ?");//获取活跃系统。
            ps.setInt(1,getAccountID());
            ps.setString(2,type);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
            ...
            log.error("错误 getActiveByType：", Ex);
        }
        return count;
    ...
    /**新增活跃*/
    public void insertActive(String activeName,int count,int type){
        /**第二天重载活跃*/
        if(!activeQueryDate.equals(DateUtil.format(new Date(),"yyyy-MM-dd"))){
            loadActive();
        }
        ...
        try {
            ...
            Connection con = DatabaseConnection.getConnection();
            PreparedStatement ps;
            ps = con.prepareStatement("insert into ActiveQuery(accid,activeName,count,activeType,today_) values (?,?,?,?,?)");//获取活跃系统。
            ps.setInt(1,getAccountID());
            ps.setString(2,activeName);
            ps.setInt(3,count);
            ps.setInt(4,type);
            ps.setString(5, DateUtil.format(new Date(),"yyyy-MM-dd"));
            ps.executeUpdate();
            /**打入缓存*/
            if(activeQueryMap.containsKey(activeName)){
                activeQueryMap.put(activeName,activeQueryMap.get(activeName)+count);
            } else {
                activeQueryMap.put(activeName,count);
            }
            ps.close();
            con.close();
        } catch (Exception Ex) {
            log.error("错误 insertActive：", Ex);
        }
    ...
    public void loadActive(){
        try {
            ...
            this.activeQueryMap = new ConcurrentHashMap<>();
            this.activeQueryDate = DateUtil.format(new Date(),"yyyy-MM-dd");/**记录活跃时间*/
            Connection con = DatabaseConnection.getConnection();
            PreparedStatement ps;
            ps = con.prepareStatement("SELECT SUM(count) as c,activeName from ActiveQuery where accid = ? and today_=? group by activeName");//获取活跃系统。
            ps.setInt(1,getAccountID());
            ps.setString(2,DateUtil.format(new Date(),"yyyy-MM-dd"));
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
...
//                activeQuery.setActiveName(rs.getString("activeName"));
//                activeQuery.setAccid(rs.getInt("accid"));
//                activeQuery.setActiveType(rs.getInt("activeType"));
                count = rs.getInt("c");
//                count = rs.getInt(1);

                activeQueryMap.put(rs.getString("activeName"),count);
            }
            rs.close();
            ps.close();
            con.close();
    ...
    /**发送每日礼物喇叭*/
    public void sendDailyGiftSpeakers(int itemId,Item item){
        Server.getInstance().broadcastMessage(getClient().getWorld(), PacketCreator.itemMegaphone(this.getName() + " : 从" + ItemInformationProvider.getInstance().getName(itemId) + "获得！大家一起恭喜他（她）吧！！！", true, getClient().getChannel(), item)); //黄色道具喇叭
    }

    /**获取师门信息*/
    public Master getMaster(int chrId){
        Connection con = null;
        Master master = null;
        try {
            con = DatabaseConnection.getConnection();
            PreparedStatement ps;
            ps = con.prepareStatement("SELECT * from master_data where cid = ?");//
            ps.setInt(1,chrId);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                master = new Master();
                ...
                master.setSchoolPoints(rs.getInt("school_points"));
                master.setTime(rs.getLong("time"));
            }
            rs.close();
            ps.close();
            con.close();
    ...
    /**获得我的师徒点*/
    public int getMyShiTuPoints(){
        return getMaster(id).getSchoolPoints();
    }

    /**创建师门*/
    public void createMaster(int cid,String name){
        try (Connection con = DatabaseConnection.getConnection();
             PreparedStatement ps = con.prepareStatement("INSERT INTO master_data(cid,`name`,school_points,time,id) VALUES (?,?,?,?,?)")) {
            ps.setInt(1,cid);
            ps.setString(2,name);
            ps.setInt(3,0);
    ...
    /**加入师门*/
    public void joinMaster(int cid,int masterId){
        try (Connection con = DatabaseConnection.getConnection();
             PreparedStatement ps = con.prepareStatement("INSERT INTO master_student(id,master_id,school_points,time) VALUES (?,?,?,?)")) {
            ps.setInt(1,cid);
    ...
    /**带徒入门-发送邀请*/
    public String sendAcceptDeclineST(Character chr){
        Master master = getMaster(chr.getId());
        Apprentice apprenticeById = findApprenticeById(chr.getId());
        MasterCache.ST_INVITE.put(chr.getId(),this);
        //邀请信息进入缓存
        if(master == null && apprenticeById == null){
            chr.getClient().getAbstractPlayerInteraction().openNpc(9330089,"师门邀请");
            return "发送邀请成功";
        }
        return "邀请发送失败";
    }
    ...
    /**通过师门查徒弟*/
    public List<Apprentice> findApprenticeByMasterId(int masterId){
        ...
        try {
            con = DatabaseConnection.getConnection();
            PreparedStatement ps;
            ps = con.prepareStatement("SELECT * from master_student where master_id = ? ORDER BY `time` ASC");//
            ps.setInt(1,masterId);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                Apprentice apprentice = new Apprentice();
                apprentice.setId(rs.getInt("id"));
                apprentice.setMasterId(masterId);
                apprentice.setSchoolPoints(rs.getInt("school_points"));
                apprentice.setTime(rs.getLong("time"));
    ...
    /**通过id查徒弟*/
    public Apprentice findApprenticeById(int cid){
        Apprentice apprentice = null;
        Connection con = null;
        try {
            con = DatabaseConnection.getConnection();
            PreparedStatement ps;
            ps = con.prepareStatement("SELECT * from master_student where id = ? ORDER BY `time` ASC");//
            ps.setInt(1,cid);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                apprentice = new Apprentice();
                apprentice.setId(rs.getInt("id"));
                apprentice.setMasterId(rs.getInt("master_id"));
                apprentice.setSchoolPoints(rs.getInt("school_points"));
                apprentice.setTime(rs.getLong("time"));
//                apprenticeList.add(apprentice);
            }
    ...
    /**通过id找等级*/
    public int findLevelById(int id){
        int level = 0;
        Connection con = null;
        try {
            con = DatabaseConnection.getConnection();
    ...
    /**删除徒弟*/
    public void delApprenticeById(int chid){
        try (Connection con = DatabaseConnection.getConnection();
             PreparedStatement ps = con.prepareStatement("DELETE FROM master_student where id=?")) {
            ps.setInt(1,chid);
            ps.executeUpdate();
        } catch (SQLException e) {
            throw new RuntimeException(e);
    ...
    /**获取师徒任务列表*/
    public List<MasterQuest> findMasterQuestList(int cid,int type,int stat){
        List<MasterQuest> masterQuestList = new ArrayList<>();
        Connection con = null;
        try {
            con = DatabaseConnection.getConnection();
            PreparedStatement ps;
            ps = con.prepareStatement("SELECT * from master_quest where cid = ? and type = ? and create_time = ? and stat = ?");//
            ps.setInt(1, cid);
            ps.setInt(2, type);
            ps.setString(3, DateUtil.format(new Date(),"yyyy-MM-dd"));
            ps.setInt(4, stat);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                MasterQuest masterQuest = new MasterQuest();
                masterQuest.setId(rs.getInt("id"));
                masterQuest.setCid(rs.getInt("cid"));
                masterQuest.setType(rs.getInt("type"));
                masterQuest.setItemid(rs.getInt("itemid"));
                masterQuest.setModid(rs.getInt("modid"));
                masterQuest.setNum(rs.getInt("num"));
                masterQuest.setMsg(rs.getString("msg"));
                masterQuest.setStat(rs.getInt("stat"));
                masterQuest.setCreateTime(rs.getString("create_time"));
                masterQuest.setFinshNum(rs.getInt("finsh_num"));
                masterQuest.setExp(rs.getInt("exp"));
                masterQuest.setShituPoints(rs.getInt("shitu_points"));
                masterQuestList.add(masterQuest);
            }
            rs.close();
            ps.close();
            con.close();
    ...
    /**获取师徒任务列表*/
    public List<MasterQuest> findMasterQuestList(int cid){
        List<MasterQuest> masterQuestList = new ArrayList<>();
        Connection con = null;
        try {
            con = DatabaseConnection.getConnection();
            PreparedStatement ps;
            ps = con.prepareStatement("SELECT * from master_quest where cid = ? and create_time = ?");//
            ps.setInt(1, cid);
            ps.setString(2, DateUtil.format(new Date(),"yyyy-MM-dd"));
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
    ...
    /**插入师徒任务*/
    public void addMasterQuest(int cid,int type,int itemid,int modid,int num,String msg,int stat,int exp,int st_points){
        try (Connection con = DatabaseConnection.getConnection();
             PreparedStatement ps = con.prepareStatement("INSERT INTO master_quest(cid,type,itemid,modid,num,msg,stat,create_time,exp,shitu_points) VALUES (?,?,?,?,?,?,?,?,?,?)")) {
            ps.setInt(1,cid);
            ps.setInt(2,type);
            ps.setInt(3,itemid);
            ps.setInt(4,modid);
            ...
            ps.setString(6,msg);
            ps.setInt(7,stat);
            ps.setString(8,DateUtil.format(new Date(),"yyyy-MM-dd"));
            ps.setInt(9,exp);
            ps.setInt(10,st_points);
//            ps.setLong(4,new Date().getTime());
            ps.executeUpdate();
        } catch (SQLException e) {
            throw new RuntimeException(e);
    ...
    /**更新师徒任务完成数量*/
    public void updateMasterQuestNum(int id,int num){
        try (Connection con = DatabaseConnection.getConnection();
             PreparedStatement ps = con.prepareStatement("update master_quest set `finsh_num`=`finsh_num`+? where id = ?")) {
            ps.setInt(1,num);
            ps.setInt(2,id);
            ps.executeUpdate();
        } catch (SQLException e) {
    ...
    /**更新师徒任务完成状态*/
    public void updateMasterQuestStat(int id,int stat){
        try (Connection con = DatabaseConnection.getConnection();
             PreparedStatement ps = con.prepareStatement("update master_quest set `stat`=? where id = ?")) {
//             PreparedStatement ps = con.prepareStatement("update master_quest set `stat`=? where id = ?")) {
            ps.setInt(1,stat);
            ps.setInt(2,id);
            ps.executeUpdate();
    ...
    /**添加师徒点*/
    public void addShiTuPoints(int id,int num,int tdid){
        /**添加师门的*/
        try (Connection con = DatabaseConnection.getConnection();
             PreparedStatement ps = con.prepareStatement("update master_data set `school_points`=school_points+? where id = ?")) {
//             PreparedStatement ps = con.prepareStatement("update master_quest set `stat`=? where id = ?")) {
            ps.setInt(1,num);
            ps.setInt(2,id);
            ps.executeUpdate();
        ...
        if(tdid != 0){
            /**徒弟个人的*/
            try (Connection con = DatabaseConnection.getConnection();
                 PreparedStatement ps = con.prepareStatement("update master_student set `school_points`=school_points+? where id = ?")) {
//             PreparedStatement ps = con.prepareStatement("update master_quest set `stat`=? where id = ?")) {
                ps.setInt(1,num);
                ps.setInt(2,tdid);
                ps.executeUpdate();
