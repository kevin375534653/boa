        ...
        if (c.tryacquireClient()) {
            try {
                ...
                if (sendMessage != null && sendMessage.length() > 100) {
                    AutobanFactory.PACKET_EDIT.alert(c.getPlayer(), c.getPlayer().getName() + " 尝试在杜宜上使用快速交付进行数据包编辑");
                    log.warn("角色 {} 试图在过长的文本中使用杜宜", c.getPlayer().getName());
                ...
                if (!quick) {
                    ...
                    AutobanFactory.PACKET_EDIT.alert(c.getPlayer(), c.getPlayer().getName() + " 尝试在杜宜上使用快速交付进行数据包编辑.");
                    log.warn("角色 {} tried to use duey with Quick Delivery without a ticket, mesos {} and amount {}", c.getPlayer().getName(), sendMesos, amount);
                    c.disconnect(true, false);
                    return;
                }
