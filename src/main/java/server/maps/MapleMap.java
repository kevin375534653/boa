...
public class MapleMap {
    ...
    private final Lock mapRLock;
    private final Lock mapWLock;
    ...
    public MapleMap(int mapid, int world, int channel, int returnMapId, float monsterRate) {
        ...
        mapWLock = maplock.writeLock();
        mapRLock = maplock.readLock();
    ...
    private byte dropItemsFromMonsterOnMap(List<MonsterDropEntry> dropEntry, Point pos, byte d, int chRate, byte droptype, int mobpos, Character chr, Monster mob) {
        ...
        for (final MonsterDropEntry de : dropEntry) {
             ...
             * 45级一下双倍掉落
             * */
            if(chr.getLevel() < 45 && de.itemId >= 1000000 && de.itemId < 2000000){
                dropChance*=2;
            }
...
//        chRate *=3;
    ...
    public void broadcastBalrogVictory(String leaderName) {
        getWorldServer().dropMessage(6, "咳……你们比我想象的要强大！不要以为你们已经将我击败。这是封印的力量……等我恢复我真正的力量，我将重新支配这个世界。");
    }

    public void broadcastHorntailVictory() {
        getWorldServer().dropMessage(6, "经过无数次的挑战，终于击破了暗黑龙王的远征队！你们才是龙之林的真正英雄~");
    }
    ...
    public void broadcastPinkBeanVictory(int channel) {
        getWorldServer().dropMessage(6, "凭借永不疲倦的热情打败品克缤的远征队啊！你们是真正的时间的胜者！");
    }
            ...
            if (removeKilledMonsterObject(monster)) {
                try {
                    if (monster.getStats().getLevel() >= chr.getLevel() + 30 && !chr.isGM()) {
                        AutobanFactory.GENERAL.alert(chr, " 杀了高出等级30级的怪物 " + monster.getName());
                    }
                    /**杀怪活跃 m_5+mob_hy_*/
                    if(monster.getStats().getLevel() > chr.getLevel() - 5){
                        int killMobHy = chr.getActiveByName("m_5+mob_hy_");/**获取当前杀怪活跃*/
                        if(killMobHy < 300){
                            /**给杀怪活跃*/
                            chr.insertActive("m_5+mob_hy_",1,0);
                            chr.dropMessage(5,"获得杀怪活跃1点");
                        }
                    }

                    /**boss远征活跃*/
                    if(monster.getId() == 8830000){//蝙蝠怪远征
                        List<Character> allPlayers = chr.getMap().getAllPlayers();
                        for (Character chrs:allPlayers) {
                            if(chrs.getActiveByName("p_bfgyz_hy_") < 100){
                                chrs.insertActive("p_bfgyz_hy_",100,0);
                                chrs.dropMessage(5,"完成蝙蝠怪远征获得100活跃点");
                                chrs.dropMessage(5,"击杀次数+1（每天有限制）");
                                chrs.setBossLog("蝙蝠怪普通");
                            }
                        }
                    }
                    ...
                    /**师徒任务杀怪 包括队友*/
                    if(null == chr.getParty()){
                        List<MasterQuest> masterQuestList = chr.findMasterQuestList(chr.getId(), 1, 0);
                        for (MasterQuest masterQuest:masterQuestList) {
                            if(masterQuest.getModid() == monster.getId()){
                                chr.updateMasterQuestNum(masterQuest.getId(),1);//杀怪+1
                                chr.dropMessage(5,masterQuest.getMsg()+" ["+(masterQuest.getFinshNum()+1)+"/"+masterQuest.getNum()+"]");
                                if((masterQuest.getFinshNum()+1) >= masterQuest.getNum()){
                                    chr.updateMasterQuestStat(masterQuest.getId(),1);//设置完成状态
                                    chr.dropMessage(5,masterQuest.getMsg()+" [完成]");
                                }
                            }
                        }
                    } else {
                        for (PartyCharacter partyCharacter:chr.getParty().getPartyMembersOnline()) {
                            Character player = partyCharacter.getPlayer();
                            if(player.getMapId() == chr.getMapId()){
                                List<MasterQuest> masterQuestList = player.findMasterQuestList(player.getId(), 1, 0);
                                for (MasterQuest masterQuest:masterQuestList) {
                                    if(masterQuest.getModid() == monster.getId()){
                                        player.updateMasterQuestNum(masterQuest.getId(),1);//杀怪+1
                                        player.dropMessage(5,masterQuest.getMsg()+" ["+(masterQuest.getFinshNum()+1)+"/"+masterQuest.getNum()+"]");
                                        if((masterQuest.getFinshNum()+1) >= masterQuest.getNum()){
                                            player.updateMasterQuestStat(masterQuest.getId(),1);//设置完成状态
                                            player.dropMessage(5,masterQuest.getMsg()+" [完成]");
                                        }
                                    }
                                }
                            }
