...
public class CashShop {
    public static class CashItem {
        ...
        /**不限制日期列表*/
        public static List<Integer> NoPeriod = new ArrayList<>();
        ...
        private CashItem(int sn, int itemId, int price, long period, short count, boolean onSale) {
            /**添加不限制日期物品*/
            if(NoPeriod.size() == 0){
                NoPeriod.add(5230000);
                NoPeriod.add(5072000);
                NoPeriod.add(5030000);
                NoPeriod.add(5150040);
                NoPeriod.add(5450000);
                NoPeriod.add(5470000);
                NoPeriod.add(5130000);
                NoPeriod.add(5121009);
                NoPeriod.add(1002850);
                NoPeriod.add(2022490);
                NoPeriod.add(5510000);
                NoPeriod.add(5041000);//高级传送石头
            }
            this.sn = sn;
            this.itemId = itemId;
            this.price = price;
    ...
    public List<Pair<Item, String>> loadGifts() {
        ...
        try (Connection con = DatabaseConnection.getConnection()) {
            ...
            /**获取账号礼物列表*/
            try (PreparedStatement ps = con.prepareStatement("SELECT * FROM `giftacc` WHERE `toacc` = ?")) {
                ps.setInt(1, accountId);
            ...
            try (PreparedStatement ps = con.prepareStatement("DELETE FROM `giftacc` WHERE `toacc` = ?")) {
