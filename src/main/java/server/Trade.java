...
public class Trade {
    ...
    private void completeTrade() {
        ...
        if (exchangeMeso > 0) {
            ...
            if (fee > 0) {
                chr.dropMessage(1, "交易成功了。扣除手续费后，获得了" + GameConstants.numberWithCommas(exchangeMeso - fee) + "金币。请确认。");
            } else {
        ...
        if (local.checkCompleteHandshake()) {
            ...
            if (local.getChr().getLevel() < 15) {
                if (local.getChr().getMesosTraded() + local.exchangeMeso > 1000000) {
                    ...
                    local.getChr().sendPacket(PacketCreator.serverNotice(1, "等级低于15级的角色每天的交易量不能超过100万金币。"));
                    return;
                } else {
                ...
                if (partner.getChr().getMesosTraded() + partner.exchangeMeso > 1000000) {
                    ...
                    partner.getChr().sendPacket(PacketCreator.serverNotice(1, "等级低于15级的角色每天的交易量不能超过100万金币。"));
                    return;
                } else {
