...
public class Expedition {
    ...
    public void beginRegistration() {
        ...
        if (!silent) {
            startMap.broadcastMessage(leader, PacketCreator.serverNotice(6, "[远征队] " + leader.getName() + " 成为远征队长。想要参加远征队的人，现在可以加入了。"), false);
            leader.sendPacket(PacketCreator.serverNotice(6, "[远征队] 你已成为远征队长，请召集足够的远征队员，然后与NPC对话开始。"));
        }
    ...
    private void scheduleRegistrationEnd() {
        ...
        schedule = TimerManager.getInstance().schedule(() -> {
            if (registering) {
                ...
                if (!silent) {
                    startMap.broadcastMessage(PacketCreator.serverNotice(6, "[远征队] 已达到时间限制，远征队已经解散。"));
                }
    ...
    public void start() {
        ...
        broadcastExped(PacketCreator.removeClock());
        if (!silent) {
            broadcastExped(PacketCreator.serverNotice(6, "[远征队] 远征队长进入了当前地图，请在远征队组建时间结束之前进入地图。"));
        }
        startTime = System.currentTimeMillis();
        //Server.getInstance().broadcastGMMessage(startMap.getWorld(), PacketCreator.serverNotice(6, "[远征队] " + type.toString() + " 远征队已经开始，队长： " + leader.getName()));
    }

    public String addMember(Character player) {
        if (!registering) {
            return "抱歉，这次远征队已经开始了，关闭了申请加入！";
        }
        if (banned.contains(player.getId())) {
            return "你被禁止参加这次远征了 #b" + leader.getName() + "#k。";
        }
        if (members.size() >= this.getMaxSize()) { //Would be a miracle if anybody ever saw this
            return "对不起，这次远征队已经满员了！";
        }
        ...
        if (!ExpeditionBossLog.attemptBoss(player.getId(), channel, this, false)) {    // thanks Conrad, Cato for noticing some expeditions have entry limit
            return "你已经达到了参加远征队的次数！请改天再试。";
        }

        members.put(player.getId(), player.getName());
        player.sendPacket(PacketCreator.getClock((int) (startTime - System.currentTimeMillis()) / 1000));
        if (!silent) {
            broadcastExped(PacketCreator.serverNotice(6, "[远征队] " + player.getName() + " 加入了远征队！"));
        }
    ...
    public boolean removeMember(Character chr) {
        if (members.remove(chr.getId()) != null) {
            ...
            if (!silent) {
                ...
                chr.dropMessage(6, "[远征队] 你已经离开了远征队。");
            }
            return true;
        }
    ...
    public void ban(Entry<Integer, String> chr) {
        ...
        if (!banned.contains(cid)) {
            ...
            if (!silent) {
                broadcastExped(PacketCreator.serverNotice(6, "[远征队] " + chr.getValue() + " 已被禁止参加远征队。"));
            }
            ...
            if (player != null && player.isLoggedinWorld()) {
                ...
                if (!silent) {
                    player.dropMessage(6, "[远征队] 你已被禁止参加远征队。");
                }
