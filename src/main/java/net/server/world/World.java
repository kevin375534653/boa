...
public class World {
    ...
    public void buddyChanged(int cid, int cidFrom, String name, int channel, BuddyOperation operation) {
        ...
        if (addChar != null) {
            ...
            switch (operation) {
                case ADDED:
                    if (buddylist.contains(cidFrom)) {
                        buddylist.put(new BuddylistEntry(name, "群未定", cidFrom, channel, true));
                        addChar.sendPacket(PacketCreator.updateBuddyChannel(cidFrom, (byte) (channel - 1)));
                    }
                    break;
                case DELETED:
                    if (buddylist.contains(cidFrom)) {
                        buddylist.put(new BuddylistEntry(name, "群未定", cidFrom, (byte) -1, buddylist.get(cidFrom).isVisible()));
    ...
    /**是否有商店*/
    public boolean hasHiredMerchant(int ownerid) {
            return activeMerchants.containsKey(ownerid);
    }
