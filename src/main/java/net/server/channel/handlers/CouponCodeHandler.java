            ...
            try (Connection con = DatabaseConnection.getConnection()) {
                try (PreparedStatement ps = con.prepareStatement("SELECT * FROM nxcode WHERE code = ?")) {
                    ...
                    try (ResultSet rs = ps.executeQuery()) {
                        ...
                        if (rs.getLong("expiration") != 0 && rs.getLong("expiration") < Server.getInstance().getCurrentTime()) {
                    ...
                    for (Pair<Integer, Pair<Integer, Integer>> pair : codeRes.getRight()) {
                        ...
                        switch (type) {
                            ...
                            default:
                                ...
                                Item i = new Item(item, (short) 0, qty);

                                cs.addToInventory(i);
                                c.sendPacket(PacketCreator.showBoughtCashItem(i, c.getAccID()));
