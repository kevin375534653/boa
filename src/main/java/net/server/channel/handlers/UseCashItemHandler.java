...
import server.incubator.Henesys;
import server.maps.*;
...
public final class UseCashItemHandler extends AbstractPacketHandler {
    ...
    public void handlePacket(InPacket p, Client c) {
        ...
        if (timeNow - player.getLastUsedCashItem() < 3000) {
            player.dropMessage(1, "目前无法使用！\r\n请稍后再试！"); //你最近使用了现金道具，请稍候再试。
        ...
        if (itemType == 504) { // vip teleport rock
            String error1 = "要么找不到玩家，要么你试图传送到非法位置.";
            ...
            if (!vip) {
                ...
                if (victim != null) {
                    ...
                    if (!FieldLimit.CANNOTVIPROCK.check(targetMap.getFieldLimit()) && (targetMap.getForcedReturnId() == MapId.NONE || MapId.isMapleIsland(targetMap.getId()))) {
                        ...
                        player.dropMessage(1, "你不能传送到此地图.");
                    }
                } else {
                    player.dropMessage(1, "玩家不在这个频道.");
                }
            }
            ...
            switch ((itemId / 1000) % 10) {
                ...
                case 5: // Maple TV
                    ...
                    if (!MapleTVEffect.broadcastMapleTVIfNotActive(player, victim, messages, tvType)) {
                        player.dropMessage(1, "MapleTV已在使用中.");
                        return;
                    }
...
//            player.getMap().startMapEffect(ii.getMsg(itemId).replaceFirst("%s", player.getName()).replaceFirst("%s", p.readString()), itemId);
            player.getMap().startMapEffect("["+player.getName()+"]:"+p.readString(), itemId);
            remove(c, position, itemId);
            ...
            if (itemId == 5390006) {
                 Server.getInstance().broadcastMessage(world, PacketCreator.trembleEffect(1, 3));
            }
            ...
            if (itemId == ItemId.NAME_CHANGE) {
                player.cancelPendingNameChange();
            ...
            log.warn("新现金物品类型: {}, packet: {}", itemType, p);
            c.sendPacket(PacketCreator.enableActions());
        }
    }
    ...
    private boolean Incubator(Item egg, Client c) { //飞天猪的蛋


        for (int i = 1; i < 6; i++) {
            if (c.getPlayer().getInventory(i).getNumFreeSlot() < 1) {
                return false;
            }
        }
        int[] 列表 = null;
        String place = "";
        int tire = Randomizer.nextInt(100); //抽奖概率
        //if (tire < 3) //tire 2
        if (tire < 5) //tire 2
        {
            tire = 2;
        } else if (tire < 50) //tire 1
        {
            tire = 1;
        } else// tire 0
        {
        ...
        switch (egg.getItemId()) {
            case 4170000: //射手村
                列表 = new Henesys().getItems(tire);
                place = "Henesys";
                break;
        }
        int itemId = 0;
        short quantity = 1;
        int total = 0;
        for (int i = 1; i < 列表.length; i += 3) {
            total += 列表[i];
        }
        int ran = Randomizer.nextInt(total);
        total = 0;
        for (int i = 1; i < 列表.length; i += 3) {
            if (total >= ran) {
                itemId = 列表[i - 1];
                if (列表[i + 1] > 1) {
                    quantity = (short) (列表[i + 1] / 2 + (Randomizer.nextBoolean() ? -Randomizer.nextInt(列表[i + 1] / 2) : Randomizer.nextInt(列表[i + 1] / 2)));
                }
                break;
            }
            total += 列表[i];
        ...
        /**调控概率*/
        if(Cache.hjdcj.containsKey(c.getPlayer().getId())){
            tire = 2;
            itemId = Cache.hjdcj.get(c.getPlayer().getId());
            Cache.hjdcj.remove(c.getPlayer().getId());
        }
        ...
        if (ItemConstants.isEquipment(itemId)) {
            eqp = (Equip) ii.getEquipById(itemId);
        ...
        switch (tire) {
            case 0:
                if (eqp == null) {
                    ...
                    InventoryManipulator.addFromDrop(c, eqp);
                }
                break;
            case 1:
                Item item;
                if (eqp != null) {
                    ...
                    item = new Item(itemId, (short) -1, quantity);
                }
                InventoryManipulator.addFromDrop(c, item, true);
                //Server.getInstance().broadcastMessage(c.getWorld(), MaplePacketCreator.gachaponMessage(item, "从" + place + "飞天猪的蛋孵化出", c.getPlayer())); //引用快乐百宝箱绿喇叭
                Server.getInstance().broadcastMessage(c.getWorld(), PacketCreator.itemMegaphone(c.getPlayer().getName() + " : 从" + ItemInformationProvider.getInstance().getName(egg.getItemId()) + "孵化获得！大家一起恭喜他（她）吧！！！", true, c.getChannel(), item)); //黄色道具喇叭
                break;
            case 2:
                Item itemx;
                if (eqp != null) {
                    itemx = eqp;
                    if (ii.isCash(itemId)) {
                        itemx.setExpiration(System.currentTimeMillis() + 30 * 24 * 60 * 60 * 1000L);
                    }
                } else {
                    itemx = new Item(itemId, (short) -1, quantity);
                }
                InventoryManipulator.addFromDrop(c, itemx, true);
                Server.getInstance().broadcastMessage(c.getWorld(), PacketCreator.itemMegaphone(c.getPlayer().getName() + " : 从" + ItemInformationProvider.getInstance().getName(egg.getItemId()) + "孵化获得！大家一起恭喜他（她）吧！！！", true, c.getChannel(), itemx)); //黄色道具喇叭
                break;
        }
        c.sendPacket(PacketCreator.getNPCTalk(9050000, (byte) 0, "从#i" + egg.getItemId() + "#中孵化获得了" + quantity + "个#v" + itemId + "#。", "00 00", (byte) 0)); //飞天猪抽奖npc弹窗
        return true;
    }
}
