            ...
            try {
                ...
                if (action == 0x03 || action == 0x1E) {
                    ...
                    if (!canBuy(chr, cItem, cs.getCash(useNX))) {
                        log.error("拒绝使用SN销售现金项目 {}", snCS); // preventing NPE here thanks to MedicOP
                        c.enableCSActions();
                        return;
                    }

                    /**只能用活跃点券购买活跃用品*/
                    if(snCS/10000000 == 4 && useNX != 2){
                        chr.dropMessage(1, "活跃用品只能使用活跃点券购买.");
                        c.enableCSActions();
                        return;
                    }

                    /**只能点券购买商品*/
                    int[] onlyNxBugs = {50200005,50200004,50200003,10000617,//百宝箱
                            50200096,50200097,50200098,//高级快乐百宝箱
                            50200150,//点券类型经验卡
                            50200128,//原地复活术
                            30000008,30000009,//高级传送石
                            50000283,//皇家自选
                            50200129,50200130,50200131,//孵蛋器
                            30200051,30200052,//浪漫樱花
                            30200033,30200032,//巧克力糖果
                            30200044,30200043,//圣诞树铃铛
                            30200040,30200039,//雪孩子

                    };//NX
                    for (int sn:onlyNxBugs) {
                        if(sn == snCS && useNX != 4){
                            chr.dropMessage(1, "这个商品只能使用点券购买.");
                            c.enableCSActions();
                            return;
                        }
                    }
                    ...
                    int[] oneDayBuyOnes = {
                            50200069
//                            50200150,50200069,40006011//双倍经验卡
                    };//NX
                    for (int sn:oneDayBuyOnes) {
                        if(sn == snCS){
                            if(chr.getBossLog("CS_Bug"+snCS) > 0){
                                chr.dropMessage(1, "商品一天只能购买一次.");
                                c.enableCSActions();
                                return;
                            } else {
                                chr.setBossLog("CS_Bug"+snCS);
                            }
                    ...
                    String noteMessage = chr.getName() + " 给你送了一份礼物！去商城看看.";
    ...
    private static boolean canBuy(Character chr, CashItem item, int cash) {
        if (item != null && item.isOnSale() && item.getPrice() <= cash) {
            log.debug("角色 {} 购买物品 {} (SN {}) 花费 {}", chr, ItemInformationProvider.getInstance().getName(item.getItemId()), item.getSN(), item.getPrice());
            return true;
        } else {
            return false;
        }
