...
public final class ItemRewardHandler extends AbstractPacketHandler {
    ...
    public final void handlePacket(InPacket p, Client c) {
        ...
        /**某些物品读脚本 投票小礼物 2022490*/
        int[] goScriptsItems = {2022490,//投票小礼物
                2022491//新手礼包
        };
        for (int goScriptsItem:goScriptsItems) {
            if(goScriptsItem==itemId){
                if (c.getCM() != null) {
                    return;
                }
                c.removeClickedNPC();
                NPCScriptManager.getInstance().dispose(c);
                NPCScriptManager.getInstance().itemStart(c, itemId);
                return;
            }
        }
