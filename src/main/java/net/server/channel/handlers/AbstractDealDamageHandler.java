        ...
        public StatEffect getAttackEffect(Character chr, Skill theSkill) {
            ...
            if (display > 80) { //Hmm
                if (!mySkill.getAction()) {
                    AutobanFactory.快速进攻.autoban(chr, "修改wz; 为技能添加动作: " + display);
                    return null;
                }
            }
        ...
        try {
            ...
            if (attack.skill != 0) {
                ...
                if (player.getMp() < attackEffect.getMpCon()) {
                    AutobanFactory.锁MP.addPoint(player.getAutobanManager(), "Skill: " + attack.skill + "; 玩家 MP: " + player.getMp() + "; MP 需要: " + attackEffect.getMpCon());
                }
                ...
                if (attack.numAttacked > mobCount) {
                    AutobanFactory.怪物数量.autoban(player, "技能: " + attack.skill + "; 攻击数量: " + attack.numAttacked + " 最大攻击数量: " + attackEffect.getMobCount());
                    return;
                }
            }
            if (!player.isAlive()) {
            ...
            for (Integer oned : attack.allDamage.keySet()) {
                ...
                if (monster != null) {
                    ...
                    if (distance > distanceToDetect) {
                        AutobanFactory.距离外挂.alert(player, "技能距离怪物: " + distance + " 技能ID: " + attack.skill + " 怪物ID: " + monster.getId());
                    ...
                    if (attack.skill != 0) {
                        if (attackEffect.getFixDamage() != -1) {
                            if (totDamageToOneMonster != attackEffect.getFixDamage() && totDamageToOneMonster != 0) {
                                AutobanFactory.固定伤害.autoban(player, totDamageToOneMonster + " 伤害");
                            }
                    ...
                    if (attack.skill == Marksman.SNIPE) {
                        if (!monster.isBoss()) {
                            totDamageToOneMonster = monster.getHp();
                        }
        ...
        for (int i = 0; i < ret.numAttacked; i++) {
            ...
            for (int j = 0; j < ret.numDamage; j++) {
                ...
                if (damage > maxWithCrit * 1.5) {
                    AutobanFactory.攻击外挂.alert(chr, "攻击: " + damage + " 最大攻击: " + maxWithCrit + " 技能id: " + ret.skill + " 怪物id: " + (monster != null ? monster.getId() : "null") + " 地图: " + chr.getMap().getMapName() + " (" + chr.getMapId() + ")");
                }

                // Add a ab point if its over 5x what we calculated.
                if (damage > maxWithCrit * 5) {
                    AutobanFactory.攻击外挂.addPoint(chr.getAutobanManager(), "攻击: " + damage + " 最大攻击: " + maxWithCrit + " 技能id: " + ret.skill + " 怪物id: " + (monster != null ? monster.getId() : "null") + " 地图: " + chr.getMap().getMapName() + " (" + chr.getMapId() + ")");
                }
