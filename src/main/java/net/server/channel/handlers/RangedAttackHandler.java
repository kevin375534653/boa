        ...
        if (attack.skill == Buccaneer.ENERGY_ORB || attack.skill == ThunderBreaker.SPARK || attack.skill == Shadower.TAUNT || attack.skill == NightLord.TAUNT) {
            ...
            for (short i = 1; i <= inv.getSlotLimit(); i++) {
                ...
                if (item != null) {
                    ...
                    if (c.getPlayer().getInventory(InventoryType.USE).getItem(slot).getItemId() == 2070018) {
                        Item bullet = c.getPlayer().getInventory(InventoryType.USE).getItem(slot);
                        short quantity = bullet.getQuantity();
                        if (quantity < 10) {
                            ItemInformationProvider ii = ItemInformationProvider.getInstance();
                            InventoryManipulator.removeFromSlot(c, InventoryType.USE, slot,(short)( -ii.getSlotMax(c, bullet.getItemId()) + quantity), false, true);
                        }
                    }
